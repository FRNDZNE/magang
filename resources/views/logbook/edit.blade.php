...
@section('breadcrumb')
    <li class="breadcrumb-item active">Edit Logbook Magang</li>
@endsection
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Edit Logbook Magang</h3>
        </div>
        <form action="{{ route('interns.logbooks.update', [$intern->uuid, $logbook->id]) }}" method="post">
            @csrf
            @method('PUT')
            <div class="card-body">
                ...
                <div class="form-group m-3">
                    ...
                    <textarea name="activity" id="activity" cols="30" rows="3"
                        class="form-control @error('activity') is-invalid @else @if (old('activity')) is-valid @endif @enderror"
                        value="{{ old('activity') }}">{{ $logbook->activity }}</textarea>
                    @error('activity')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @else
                        @if (old('activity'))
                ...
                <div class="form-group m-3">
                    ...
                    <textarea name="output" id="output" cols="30" rows="3"
                        class="form-control @error('output') is-invalid @else @if (old('output')) is-valid @endif @enderror">{{ $logbook->output }}</textarea>
                    @error('output')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @else
                        @if (old('output'))
